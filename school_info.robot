*** Settings ***
Library    Autosphere.Browser          # Used for browser automation
...
Library    school_excel_helper.py          # Custom Python helper to save Excel data
...
School Automation
    ...
    ensure excel downloaded    ${EXCEL_PATH}
    ...
    FOR    ${row}    IN    @{rows}
        ${school_code}=    Evaluate    '' if pandas.isna($row['School Code']) else str($row['School Code']).strip()    modules=pandas
        IF    '${school_code}'==''
            CONTINUE
        END
